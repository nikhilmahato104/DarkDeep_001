      ...
      <section className="py-20 bg-stone-50">
        <div className="container mx-auto px-4 max-w-4xl">
                ...
                {category.items.map((item, itemIdx) => (
                  <div key={itemIdx} className="flex justify-between items-end group">
                    <div className="flex-grow">
                      <span className="font-medium text-lg text-stone-700 group-hover:text-primary transition-colors">{item.name}</span>
                      <div className="border-b border-dotted border-stone-300 w-full mb-1 opacity-50 hidden md:block" />
                    </div>
                    <span className="font-bold text-lg text-stone-900 ml-4 whitespace-nowrap">{item.price}</span>
                  </div>
                ))}
              </div>
            </motion.div>
          ...
          <div className="text-center mt-12 bg-primary/10 p-8 rounded-sm">
            <h3 className="font-display text-2xl text-stone-900 mb-4">Membership Packages Available</h3>
            <p className="text-stone-600 mb-6">Ask our front desk about our monthly membership plans and save up to 20% on all treatments.</p>
            <Link href="/contact">
              <Button size="lg" className="bg-primary hover:bg-primary/90 text-white">Inquire Now</Button>
            </Link>
          </div>
        </div>
      </section>
